...
class FactionListener implements Listener {
    ...
    public function factionChat(PlayerChatEvent $PCE) {
        ...
        if ($this->plugin->motdWaiting($player)) {
            ...
            $PCE->getPlayer()->sendMessage($this->plugin->formatMessage(TextFormat::GRAY . "Timed out. Please use /f desc again."));
                ...
                $PCE->setCancelled(true);
                $PCE->getPlayer()->sendMessage($this->plugin->formatMessage(TextFormat::GRAY . "Successfully updated the faction description. Type /f info.", true));
            }
            return true;
        }
                ...
                foreach ($this->plugin->getServer()->getOnlinePlayers() as $fP) {
                    if ($this->plugin->getPlayerFaction($fP->getName()) == $faction) {
                        if ($this->plugin->getServer()->getPlayer($fP->getName())) {
                            $PCE->setCancelled(true);
                            $this->plugin->getServer()->getPlayer($fP->getName())->sendMessage(TextFormat::GRAY . "[" . TextFormat::AQUA . TextFormat::BOLD . "FC" . TextFormat::RESET . TextFormat::GRAY . "]" . TextFormat::AQUA . " $player: " . TextFormat::GRAY . $msg);
                        }
                    }
                }
            }
            ...
            if ($this->plugin->allyChatActive[$player]) {
                ...
                foreach ($this->plugin->getServer()->getOnlinePlayers() as $fP) {
                    if ($this->plugin->areAllies($this->plugin->getPlayerFaction($fP->getName()), $faction)) {
                        if ($this->plugin->getServer()->getPlayer($fP->getName())) {
                            $PCE->setCancelled(true);
                            $this->plugin->getServer()->getPlayer($fP->getName())->sendMessage(TextFormat::GRAY . "[" . TextFormat::YELLOW . TextFormat::BOLD . "AC" . TextFormat::RESET . TextFormat::GRAY . "]" . TextFormat::AQUA . " $player: " . TextFormat::GRAY . $msg);
                            $PCE->getPlayer()->sendMessage(TextFormat::GRAY . "[" . TextFormat::YELLOW . TextFormat::BOLD . "AC" . TextFormat::RESET . TextFormat::GRAY . "]" . TextFormat::AQUA . " $player: " . TextFormat::GRAY . $msg);
                        }
                    }
                }
            }
    ...
    public function factionBlockBreakProtect(BlockBreakEvent $event) {
        ...
        if ($this->plugin->pointIsInPlot($x, $z, $level)) {
            if ($this->plugin->factionFromPoint($x, $z, $level) === $this->plugin->getFaction($event->getPlayer()->getName())) {
                ...
                $event->getPlayer()->sendMessage($this->plugin->formatMessage(TextFormat::GRAY . "You cannot break blocks here. Type /f plotinfo for details."));
                return;
            }
        }
    }
    ...
    public function factionBlockPlaceProtect(BlockPlaceEvent $event) {
        ...
        if ($this->plugin->pointIsInPlot($x, $z, $level)) {
            if ($this->plugin->factionFromPoint($x, $z, $level) == $this->plugin->getFaction($event->getPlayer()->getName())) {
                ...
                $event->setCancelled(true);
                $event->getPlayer()->sendMessage($this->plugin->formatMessage(TextFormat::GRAY . "You cannot place blocks here. Type /f plotinfo for details."));
                return;
            }
        }
    }
