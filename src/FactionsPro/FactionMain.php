...
class FactionMain extends PluginBase implements Listener {
    ...
    public function getPlayersInFactionByRank($s, $faction, $rank) {
        ...
        while ($resultArr = $result->fetchArray(SQLITE3_ASSOC)) {
            ...
            if ($this->getServer()->getPlayerExact($row[$i]['player']) instanceof Player) {
                ...
                $team .= TextFormat::ITALIC . TextFormat::AQUA . $row[$i]['player'] . TextFormat::GRAY . "[OFF]" . TextFormat::RESET . TextFormat::WHITE . "||" . TextFormat::RESET;
            }
            $i = $i + 1;
        }
    ...
    public function getAllAllies($s, $faction) {
        ...
        if ($i > 0) {
            $s->sendMessage($this->formatMessage(TextFormat::GRAY . "Allies of $faction", true));
            $s->sendMessage($team);
        } else {
            $s->sendMessage($this->formatMessage(TextFormat::GRAY . "$faction has no allies", true));
        }
    }
    ...
    public function sendListOfTop10FactionsTo($s) {
        ...
        $i = 0;
        $s->sendMessage($this->formatMessage(TextFormat::BOLD . TextFormat::AQUA . "Top 10 strongest factions", true));
        while ($resultArr = $result->fetchArray(SQLITE3_ASSOC)) {
            $j = $i + 1;
            $cf = $resultArr['faction'];
            $pf = $this->getFactionPower($cf);
            $df = $this->getNumberOfPlayers($cf);
            $s->sendMessage(TextFormat::ITALIC . TextFormat::GRAY . "$j -> " . TextFormat::AQUA . "$cf" . TextFormat::GRAY . " with " . TextFormat::AQUA . "$pf power" . TextFormat::GRAY . " and " . TextFormat::AQUA . "$df members" . TextFormat::RESET);
            $i = $i + 1;
        }
    }
        ...
        if ($this->cornerIsInPlot($x + $arm, $z + $arm, $x - $arm, $z - $arm, $level->getName())) {
            ...
            if ($this->prefs->get("EnableOverClaim")) {
                if ($power_sender < $power_claimedBy) {
                    $sender->sendMessage($this->formatMessage(TextFormat::GRAY . "This area is claimed by $claimedBy with $power_claimedBy Power. Your faction has $power_sender power. You don't have enough power to overclaim this plot."));
                } else {
                    $sender->sendMessage($this->formatMessage(TextFormat::GRAY . "This area is claimed by $claimedBy with $power_claimedBy Power. Your faction has $power_sender power. Type /f overclaim to overclaim this plot."));
                }
                return false;
            } else {
                $sender->sendMessage($this->formatMessage(TextFormat::GRAY . "Overclaiming is disabled."));
                return false;
            }
        }
